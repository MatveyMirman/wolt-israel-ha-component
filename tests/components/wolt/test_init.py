...
def mock_config_entry():
    ...
    entry.data = {
        ...
        CONF_VENUES: [
            {CONF_SLUG: "test-venue", "delivery_method": "homedelivery"}
        ],
    }
    entry.options = {}
    return entry
...
async def test_async_setup_entry_success(mock_config_entry):
    ...
    assert "entry" in mock_hass.data[DOMAIN][mock_config_entry.entry_id]
...
async def test_async_unload_entry(mock_config_entry):
    ...
    mock_hass = MagicMock()
    mock_hass.data = {DOMAIN: {mock_config_entry.entry_id: {"entry": mock_config_entry, "coordinators": {}}}}
